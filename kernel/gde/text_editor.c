...
static void te_draw(gde_window_t *win)
{
    ...
    gfx_draw_string(cx+6, cy+(TE_HDR_H-GFX_CHAR_H)/2,
                    st->path, COL_TE_PATH, COL_TE_HDR, 0);
...
static void te_key(gde_window_t *win, char k)
{
    ...
    if      (k==0x26) {                              /* Up             */
        ...
        st->cursor=st->nlines>0?st->nlines-1:0;
        if (st->cursor>=visible) st->scroll=st->cursor-visible+1;
    }
}
...
void text_editor_open(const char *path)
{
    ...
    st->size=0; st->nlines=0; st->scroll=0; st->cursor=0;
